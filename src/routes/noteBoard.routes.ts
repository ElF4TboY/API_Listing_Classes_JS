...
import { NoteBoard } from "../models/noteBoard";
...
router.post("/", async (req: Request, res: Response) => {
  if (req.body) {
    const newNoteBoard = new NoteBoard<NoteBoardApi>(req.body);
    try {
...
router.get("/:id", async (req: Request, res: Response, next: NextFunction) => {
  const noteBoardId: string = req.params.id;
  try {
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const noteBoardId: string = req.params.id;
    try {
      const noteboard = await NoteBoard.findByIdAndUpdate(
        noteBoardId,
        req.body,
        {
          new: true,
